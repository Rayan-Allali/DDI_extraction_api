...
class MultiViewCrossAttentionWithEdges(nn.Module):
    def __init__(self, embed_dim, cls_dim=None):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_heads = 4
    ...
    def forward(self, Q, K, V):
        ...
        return context
edge_dim=1
class GATModelWithAttentionWithEdges(nn.Module):
    def __init__(self, node_in_dim, gat_hidden_channels, cls_dim, dropout_rate, num_classes=5):
        super().__init__()

        self.gat1 = GATv2Conv(node_in_dim, gat_hidden_channels, heads=4, dropout=dropout_rate, edge_dim=edge_dim)
        self.bn1 = nn.BatchNorm1d(gat_hidden_channels * 4)
        self.gat2 = GATv2Conv(gat_hidden_channels * 4, gat_hidden_channels, heads=4, dropout=dropout_rate, edge_dim=edge_dim)
        self.bn2 = nn.BatchNorm1d(gat_hidden_channels * 4)
        self.gat3 = GATv2Conv(gat_hidden_channels * 4, gat_hidden_channels, heads=4, dropout=dropout_rate, edge_dim=edge_dim)
        self.bn3 = nn.BatchNorm1d(gat_hidden_channels * 4)
        self.cross_attention = MultiViewCrossAttentionWithEdges(gat_hidden_channels * 4, cls_dim)
        self.ffn = nn.Sequential(
            nn.Linear(gat_hidden_channels * 4, gat_hidden_channels * 4),
            nn.ReLU(),
            nn.Dropout(p=dropout_rate),
    ...
    def forward(self, data):
        ...
        node_features = (global_add_pool(x, batch) + global_mean_pool(x, batch)) / 2
        biobert_cls = data.biobert_cls.view(-1, 768)
        attn_output = self.cross_attention(node_features, biobert_cls, biobert_cls)
        ffn_output = self.ffn(attn_output)
        attn_output = attn_output + ffn_output
